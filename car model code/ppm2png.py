...
for train_class in os.listdir(ORIGINAL_TRAIN_PATH):
    if not os.path.isdir('../car-model/train/'+train_class):
        os.mkdir('../car-model/train/'+train_class)        
    for pic in os.listdir(ORIGINAL_TRAIN_PATH + '/'+ train_class):
        ...
        new_name = name+'.png'
        print (new_name)
        cv2.imwrite('../car-model/train/' + train_class + '/'+ new_name,im)
        

for test_class in os.listdir(ORIGINAL_TEST_PATH):
    if not os.path.isdir('../car-model/test/'+test_class):
        os.mkdir('../car-model/test/'+test_class)        
    for pic in os.listdir(ORIGINAL_TEST_PATH + '/'+ test_class):
        ...
        new_name = name+'.png'
        print (new_name)
        cv2.imwrite('../car-model/test/' + test_class + '/'+ new_name,im)
